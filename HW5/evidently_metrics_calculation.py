...
def calculate_metrics_postgresql(curr, i):
	...
	quantile_values = result['metrics'][4]['result']['current']['value']

	curr.execute(
		"insert into dummy_metrics(timestamp, prediction_drift, num_drifted_columns, share_missing_values, quantiles_values) values (%s, %s, %s, %s)",
...
def batch_monitoring_backfill():
	...
	with psycopg.connect("host=localhost port=5432 dbname=test user=postgres password=mlops", autocommit=True) as conn:
		for i in range(0, 27):
